...
class Customer(Person):
    ...
    def action(self, order_item):
        # 돈이 충분할때,주문 넣기
        if (self.inventory.items['money'].quantity >= order_item.price):
...
class Barista(Person):
    ...
    def action(self, customer):
        # 재료가 없으면 종료,충분하면 +4번째 줄(for ingredient, quantity in customer.order.ingredients.items():)로 이동
        for ingredient in customer.order.ingredients.keys():
            if not customer.order.ingredients[ingredient] < self.inventory.items[ingredient].quantity:
                return
        # 재료 제거
